...
function TB_show(caption, url, rel){
    ...
    if (!$("TB_overlay")) {
        ...
        $('TB_load').innerHTML = "<img src='/thirdParty/smoothbox/loading.gif' />";
        TB_load_position();
...
/**
 * Added NM - a wrapper for convenient firing via simple URL
 * @param url, height, width, openLink - whether provide an "open in new window" link
 * @return
 */
function fireLightBox(url, height, width, openLink) {
	// defaults
	height=((height !== undefined)? height:700);
	width=((width !== undefined)? width:800);
	openLink = ((openLink !== undefined)?openLink:true);
	var ol = "";
	if (openLink){
			//ol = "<a href=" + url + " target=\"_new\" style=\"background-image: url('/images/utils/pin.jpeg');  background-repeat: no-repeat; font-size: 0.9em; font-weight: bold; padding-left: 15px;\">&nbsp;Open in new window</a>";
			//<a class='help' href='#' title='Use right-click menu to go to previous/next page'>&nbsp;</a>
			//ol = "<a href='#' onClick='historyBack();return false;'> << Back</a>&nbsp;<a href='#' onClick='historyForward();return false;'> Forward >> </a>&nbsp;&nbsp; <a href='#' onClick='goToQuery();return false;'> Go to query </a> &nbsp;&nbsp;&nbsp;&nbsp;<a href=" + url + " target=\"_new\">Open in new window</a>";
			ol = "<a href='#' onClick='historyBack();return false;'> << Back</a>&nbsp;<a href='#' onClick='historyForward();return false;'> Forward >> </a>&nbsp;&nbsp; <a href='#' onClick='goToQuery(\""+url+"\");return false;' title='Show recent query results'> Query results </a> &nbsp;&nbsp;&nbsp;&nbsp;<a href=" + url + " target=\"_blank\" title='Open recent query results in new window'>Query results in new window</a>";
	}
	var urlStart = (url.indexOf("?")>-1)?"&":"?";
    TB_show(ol, url + urlStart + "height="+height+"&width="+width+"&TB_iframe=true", false);
    return false;
}      

function historyBack(){
	var iframe = parent.TB_iframeContent;
	//alert("historyBack" + iframe);
	iframe.history.go(-1);
}
...
function goToQuery(url){
	var cookieValue = (url !== undefined && url != null && url != '')?url: Cookie.read("recentQuery");
	//alert("cookie: "+ cookieValue);
	if (cookieValue!==undefined && cookieValue!=null && cookieValue!=''){
		var iframe = parent.TB_iframeContent;
		iframe.location=cookieValue;
	}
