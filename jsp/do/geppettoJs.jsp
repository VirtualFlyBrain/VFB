...
oldSelection = "";
setInterval(function(){ selection = G.getSelection(); message = ""; if (selection.length > 0){ if (selection[0].getParent() != oldSelection){ oldSelection = selection[0].getParent(); for (i in selection){ message += "<b onclick=\"G.addWidget(1).setData("+selection[i].getParent().getId()+"."+selection[i].getParent().getId()+"_meta).setName("+selection[i].getParent().getId()+".getName()).setPosition(92,110).addCustomNodeHandler(customHandler,'click');\">" + selection[i].getParent().getName() + "</b><br />"; }; nameWid.setMessage(message); }; }; }, 3000);
