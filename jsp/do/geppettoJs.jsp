...
var setSepCol = function(path){console.log(path+" setting colour....");try{ if (typeof window[path] == "undefined") { setSepCol(path); }else{Instances.getInstance(path).setColor(colours[order[path]],true).setOpacity(0.8,true);if (order[path]<1){Instances.getInstance(path).setOpacity(0.2,true);}; capacity++; }}catch (ignore){setSepCol(path);}};
...
loading = false; timeout = []; timeout["max"] = 10; order = []; order["curr"] = 0; capacity = 2;
...
var delay=function(ms, func, param) { console.log( order[param] + "/" + total + " [" + capacity + "]"); setTimeout(function(){ if (capacity > 0) {capacity--; func(param);}else{delay(500, func, param);}}, ms);}
