...
<c:if test="${fn:length(individuals)>0}">
  <c:forEach items="${individuals}" var="curr" varStatus="status">Model.getDatasources()[0].fetchVariable("${curr}");
  </c:forEach>
  
  <c:forEach items="${individuals}" var="curr" varStatus="status">Instances.getInstance("${curr}.${curr}_meta");</c:forEach>
  
  <c:forEach items="${individuals}" var="curr" varStatus="status">setTimeout(function() { ${curr}.setColor(colours[coli]);coli++;if (coli>199) {coli=0;}; if (${curr}.getType().getWrappedObj().defaultValue.eClass == "OBJ"){${curr}.setOpacity(0.8,true);};}, 6000);
  </c:forEach>
