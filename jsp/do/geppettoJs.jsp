...
var setSepCol = function(path){ var c = coli; coli++; if (coli>199) {coli=0;};setTimeout(function() {Instances.getInstance(path).setColor(colours[c],true).setOpacity(0.8,true);if (c<1){Instances.getInstance(path).setOpacity(0.2,true);};}, 6000);};
...
oldSelection = "";
setInterval(function(){ selection = G.getSelection(); message = ""; if (selection.length > 0){ if (selection[0].getParent() != oldSelection){ oldSelection = selection[0].getParent(); for (i in selection){ message += "<b style=\"cursor: pointer;\" onclick=\"G.addWidget(1).setData("+selection[i].getParent().getId()+"."+selection[i].getParent().getId()+"_meta).setName("+selection[i].getParent().getId()+".getName()).setPosition((window.innerWidth-(Math.ceil(window.innerWidth/5)+10)),10).setSize((window.innerHeight-20),Math.ceil(window.innerWidth/5)).addCustomNodeHandler(customHandler,'click');\">" + selection[i].getParent().getName() + "</b><br />"; }; nameWid.setMessage(message); }; }; }, 3000);
