...
var setSepCol = function(path){console.log(path+" setting colour....");try{ if (typeof window[path] == "undefined") { setSepCol(path); }else{Instances.getInstance(path).setColor(colours[order[path]],true).setOpacity(0.8,true);if (order[path]<1){Instances.getInstance(path).setOpacity(0.2,true);}; capacity++; loaded++; }}catch (ignore){setSepCol(path);}};
...
loaded = 0; timeout = []; timeout["max"] = 10; order = []; order["curr"] = 0; capacity = 1;
