...
var setSepCol = function(path){ setTimeout(function() {Instances.getInstance(path).setColor(colours[coli],true);coli++;if (coli>199) {coli=0;};}, 6000);}
...
oldSelection = "";
setInterval(function(){ selection = G.getSelection(); message = ""; if (selection.length > 0){ if (selection[0].getParent() != oldSelection){ oldSelection = selection[0].getParent(); for (i in selection){ message += "<b style=\"cursor: pointer;\" onclick=\"G.addWidget(1).setData("+selection[i].getParent().getId()+"."+selection[i].getParent().getId()+"_meta).setName("+selection[i].getParent().getId()+".getName()).setPosition((window.innerWidth-270),10).setSize((window.innerHeight-20),260).addCustomNodeHandler(customHandler,'click');\">" + selection[i].getParent().getName() + "</b><br />"; }; nameWid.setMessage(message); }; }; }, 3000);
