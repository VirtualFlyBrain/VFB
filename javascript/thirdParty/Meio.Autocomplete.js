...
(function(global, $){
	...
	Meio.Autocomplete.Filter.define('itune', function(self, keys){
		return {
			filter: function(text, data){			
				var tokens = text.split(" ");
				var match = true;
				var currItem = self._getValueFromKeys(data, keys);
				for ( var i = 0; i < tokens.length; i++ )
				  {
					match = match && currItem.test(new RegExp(tokens[i].escapeRegExp(), 'i'));
				  }
				return match? currItem : false;
			}
		};
	});
