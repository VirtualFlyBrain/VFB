...
function updateWlzDisplay() {
    updateStackData();
    if ((!$('#slider-' + controlSelection + 'CurrentSlider').is(":visible")) && (!$('#searchgroup').is(":focus"))) {
...
function loadColours() {
    ...
    if (!parent.$("body").data("colours") || parent.$("body").data("colours").length < 200) {
        if (!coloursLoading) {
            coloursLoading = true;
            var file = "/data/VFB/colours200.csv";
...
function drawText(X, Y, message, color) {
    ...
    if (scl > 0.4) {
        ...
        if (check_textRenderContext(ctx)) {
            ...
            ctx.strokeText(message, X, Y, point);
        }
    }
}
        ...
        success: function (data) {
            ...
            if (oldPos == newPos) {
                ...
                for (var i = 0, l = lastSel.length; i < l; i++) {
                    if (lastSel[i] > 0) {
                        var fullItem = parent.$("body").data("current").template.replace("VFBt_", "VFBd_") + String(pad(parseInt(lastSel[i]), 5));
...
function initWlzControls() {
    ...
    if (parent.$("body").data("meta")) {
        ...
        var slSlice = $("#slider-slice").bootstrapSlider({
            precision: 0,
            tooltip: 'always',
            handle: 'triangle',
            min: 1,
            max: maxSlice,
            step: 1,
            value: Math.round((parseInt(parent.$("body").data("meta").center.split(',')[orientation[orient].D]) + 1) * parseFloat(parent.$("body").data("meta").voxel.split(',')[orientation[orient].D])),
            focus: true
        });
        slSlice.on('slide', function (ev) {
            window.reloadInterval = 8;
        ...
        var slScale = $("#slider-scale").bootstrapSlider({
            precision: 1,
            tooltip: 'always',
            handle: 'triangle',
            scale: 'logarithmic',
            min: 0.1,
            max: 5,
            value: parseFloat(parent.$("body").data("current").scl),
            step: 0.1,
            focus: true
        });
        ...
        var slAlpha = $("#slider-alpha").bootstrapSlider({
            ...
            max: 100,
            step: 1,
            value: Math.round((parseInt(parent.$("body").data("current").alpha) / 255.0) * 100.0),
            focus: true
        });
...
function createInfoButtonHTML(layer) {
    ...
    if (layer) {
        ...
        content += '"><span style="border:none;padding-left:0;padding-right:0;" class="glyphicon glyphicon-info-sign"></span></button>';
    }
    return content;
}
...
function createCloseButtonHTML(layer) {
    ...
    if (layer) {
        ...
        content += '"><span style="border:none;padding-left:0;padding-right:0;" class="glyphicon glyphicon-trash"></span></button>';
    }
    return content;
}

function createAddButtonHTML(id) {
    return '<span style="border:none;padding-left:0;padding-right:0;" id="attach" data-id="' + cleanIdforInt(id) + '"></span>';
}
...
function createAddToQueryButtonHTMLfinal(id) {
    ...
    text += '"><span style="border:none;padding-left:0;padding-right:0;" class="glyphicon glyphicon-tasks"></span></button>';
    return text;
}
...
function createCentreButtonHTML(fxp) {
    ...
    html += '"><span style="border:none;padding-left:0;padding-right:0;" class="glyphicon glyphicon-screenshot"></span></button>';
    return html;
}
...
function moveToCentre(fxp) {
    ...
    var aim = fxp.split(',')[ax];
    var size = parent.$('body').data('meta').voxel.split(',')[ax];
    parent.$('body').data('current').dst = Math.round((aim - cur) * size);
    updateWlzDisplay();
    updateMenuData();
    window.reloadInterval = 10;
...
function loadReferenceMeta(id) {
    if (id) {
        var file = "/data/" + fileFromId(id).replace("composite.wlz", "ref.json");
...
function expandTree() {
    ...
    children.show('fast');
    $('.parent_li').find(' > span').find(' > b').html('<span class="glyphicon glyphicon-triangle-bottom" style="border:none;padding-left:0;padding-right:0;"></span>');
...
function collapseTree() {
    var children = $('.tree li.parent_li > span').parent('li.parent_li').find(' > ul > li');
    children.hide('fast');
    $('.parent_li').find(' > span').find(' > b').html('<span class="glyphicon glyphicon-triangle-right" style="border:none;padding-left:0;padding-right:0;"></span>');
...
function createAddToQueryButtonHTML(id) {
    return '<span id="addToQuery" style="border:none;padding-left:0;padding-right:0;" title="Add to query" data-id="' + cleanIdforInt(id) + '"></span>';
}
...
function createTreeHTML(treeStruct) {
    ...
    for (n in treeStruct) {
        ...
        if (parent.$('body').data('domains')[id]) {
            html += '<span id="treeLabel"><b><span class="glyphicon glyphicon-menu-right" style="border:none;padding-left:0;padding-right:0;"></span></b> ' + parent.$("body").data("domains")[id].name;
            html += "</span>";
            ...
            html += '<span id="buttonsFor' + temp + '" data-id="' + temp + '" data-extid="' + cleanIdforInt($("body").data("domains")[id].extId[0]) + '" style="position:absolute;border:none;padding-left:0;padding-right:0;">';
...
$('body').ready(function () {
    ...
    $(window).on("navigate", function (event, data) {
        ...
        if (direction == 'back') {
            change = Date.now();
            console.log('previous page requested by user...');
            clearAllDisplayed();
